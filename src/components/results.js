    ...
    return (
        <div id = {DOMId} className = 'w-full flex flex-col items-center gap-base'>
            <Map items = {events} callback = {(matchup, index) => {
                ...
                <div key = {index} id = {matchupId} className = 'w-min flex flex-col items-center gap-xs'>
                    ...
                    <>
                        ...
                        <div id = {matchupId + '-scores'} className = 'flex items-center'>
                            <Map items = {matchup.competitors} callback = {(competitor, index) => {
                                ...
                                <React.Fragment key = {index}>
                                    <Conditional value = {index === 0}>
                                        <Link id = {scoreId + '-competitor0-link'} to = {'/info?category=competitors&id=' + score.competitor.id}>
                                            <Text id = {scoreId + '-competitor0'} preset = 'title' classes = {'whitespace-nowrap text-primary-main'}>
                                    ...
                                    <Conditional value = {index !== 0}>
                                        <Link id = {scoreId + '-competitor1-link'} to = {'/info?category=competitors&id=' + score.competitor.id}>
                                            <Text id = {scoreId + '-competitor1'} preset = 'title' classes = {'whitespace-nowrap text-primary-main'}>
                                                &nbsp;&nbsp;{score.competitor.name}
                                            </Text>
                                        </Link>
                                    </Conditional>
                        ...
                        <div id = {matchupId + '-bets'} className = 'w-full flex flex-col gap-xs'>
                            <Map items = {matchup.results.bets} callback = {(bet, index) => {
                                ...
                                <div key = {index} id = {betId} className = {'w-full flex items-center ' + (bet.key === 'totals' ? 'justify-center' : 'justify-between')}>
                                    {bet.key !== 'totals' && <>
