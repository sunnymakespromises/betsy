...
const Navigation = memo(function Header({ location, currentUser }) {
    ...
    if (currentUser) {
        return (
            <div id = {DOMId + 'container'} className = 'group transition-all duration-main w-full md:w-14 h-16 md:h-full flex flex-row md:flex-col justify-between md:justify-start items-center bg-base-main z-10 animate-duration-300 animate-slideInLeft'>
...
const Icon = memo(function Icon({ index, page, isDev, isCurrent, parentId }) {
    ...
    if (page.show && (page.is_dev ? isDev : true)) {
        return (
            <Link id = {DOMId + 'container'} to = {page.path} className = {'group/icon transition-all duration-main relative w-full h-full md:h-min md:aspect-square p-small flex justify-center items-center'}>
                <Icon id = {DOMId + 'icon'} title = {page.title} className = {'!w-full !h-full cursor-pointer ' + (isCurrent ? 'text-primary-main' : 'text-text-main/killed group-hover/icon:text-primary-main')}/>
            </Link>
        )
    }
